= Perpetual publishing over Zettelkasten
...
[TIP]
====
That script was dynamically added during my `build` command just by including the following line in my `blog` repo at a reliable location relative to my `dot` repo (
`include::../../dot/scripts/blog[]`).
